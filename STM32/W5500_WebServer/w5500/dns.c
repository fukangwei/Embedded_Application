...
uint8 *dns_answer ( uint8 *msg, uint8 *cp ) {
    ...
    switch ( type ) {
        ...
        case TYPE_MX:
            cp += 2;
            len = parse_name ( msg, cp, MAX_DNS_BUF_SIZE ); /* Get domain name of exchanger */
        ...
        case TYPE_SOA:
            len = parse_name ( msg, cp, MAX_DNS_BUF_SIZE ); /* Get domain name of name server */
            ...
            cp += len;
            len = parse_name ( msg, cp, MAX_DNS_BUF_SIZE ); /* Get domain name of responsible person */
