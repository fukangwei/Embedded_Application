...
static int handle_get_temperature ( coap_rw_buffer_t *scratch, const coap_packet_t *inpkt, coap_packet_t *outpkt, uint8_t id_hi, uint8_t id_lo ) {
	...
	sprintf ( out, "%.1f", temp );
	printf ( "out is %s\r\n", out );
	ret = coap_make_response ( scratch, outpkt, ( const uint8_t * ) out, strlen ( out ), id_hi, id_lo, &inpkt->tok, COAP_RSPCODE_CONTENT, COAP_CONTENTTYPE_TEXT_PLAIN );
	return ret;
}
...
const coap_endpoint_t endpoints[] =
{
    ...
    {COAP_METHOD_GET, handle_get_temperature, &path_temperature, "ct=40"},
