...
int main ( void ) {
    ...
    while ( 1 ) {
        LWIP_Polling();
#if UDP_CLIENT
        sprintf ( ( char * ) lwip_demo_buf, "This is a UdpClient\r\n" );
        lwip_flag |= LWIP_SEND_DATA;
        udp_client_send_data();
#else /* 注意，要使用UdpServer，需要修改udp_server.c中的一些注释 */
        sprintf ( ( char * ) lwip_demo_buf, "This is a UdpServer\r\n" );
        lwip_flag |= LWIP_SEND_DATA;
